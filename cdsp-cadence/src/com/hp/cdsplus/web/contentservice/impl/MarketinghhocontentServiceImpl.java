...
public class MarketinghhocontentServiceImpl extends AbstractGenericService {
	
	LinkGroomer linkGroomer = new LinkGroomer("marketinghhocontent");
...
private Ref getRefObject(Options options, String subscription,BasicDBObject dbObject) {
	...
	public Object getDocumentMetaData(Options options) throws ApplicationException, WebApplicationException {
		...
		DBCursor subListCursor;
		try {
			subListCursor = contentDAO.getAllSubscriptions("marketinghhocontent");
			DBObject dbObject = null;
			
			while(subListCursor.hasNext()){
				dbObject = subListCursor.next();
				if(options.getSubscription().equalsIgnoreCase((String)dbObject.get("_id"))){
					if(dbObject.get("isGroomer")!=null && (Boolean)dbObject.get("isGroomer")==true){
						return linkGroomer.applyGroomsToAttributes(ipstream,options,null);
					}
					else{
						return getDocumentObject(ipstream,options);
					}
		...
		*/
		...
		try {
			...
			while(nodesIterator.hasNext()){
				...
				element.addElement("proj:ref").addAttribute("xlink:type", "simple").addAttribute("xlink:href", "marketinghhocontent/"+subscription+"/"+options.getDocid()+"/"+element.attributeValue("src"));
			}
			return document.asXML();
		} catch (SAXException e) {
			e.printStackTrace();
