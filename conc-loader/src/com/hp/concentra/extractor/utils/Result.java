...
/**
 * Result is a structure that organizes data returned by a Concentra query (DQL)
 * like a table.
 * 
 * Every query to Concentra return a IDfCollection object that is taked by the
 * Result Object and transformed to a multidimensional array strutucture
 * containing the values in Java objects format, avoiding the handle of IDfValue
 * and other Concentra concepts.
 * 
...
public class Result implements Serializable {
    ...
    public Result(IDfCollection collection) {
        if (collection != null) {
            try {
                ...
                while (collection.next()) {
                    for (int index = 0; index < indexByNumber.size(); index++) {
                        ArrayList row = (ArrayList) data.get(index);
                        ...
                        if (((String) indexByNumber.get(index)).toUpperCase().endsWith(SUFFIX_BOOLEAN)
                            ...
                            if (!value.toString().equals("")) {
                                row.add(new Boolean(value.asInteger() == CONCENTRA_TRUE_VALUE));
                            } else {
                                row.add(value);
                            }
                        } else {
                            row.add(extractValue(value));
                        }
                ...
                LoaderLog.fatal("Documentum foundation exception trying to get the result list class");
            }
        }
    }
            ...
            if (!value.toString().equals("")) {
                switch (value.getDataType()) {
                    ...
                    case IDfValue.DF_TIME:
                        return new IDfTimeConcentra(value.asTime());
    ...
    public int getRowCount() {
        return (data != null && !data.isEmpty()) ? ((ArrayList) data.get(0)).size() : 0;
    }
    ...
    public IDfTime getResultDate(int row, String column) {
      Object val = getValue(row, column);
      return (val != null) ? (IDfTime)val : null;
    }
    
    public String getResultString(int row, String column) {
      Object val = getValue(row, column);
      return (val != null) ? val.toString() : null;
    }
    
    public int getResultInt(int row, String column) throws NumberFormatException {
      Object val = getValue(row, column);
      if (val != null && (val instanceof Integer)) {
        return ((Integer)val).intValue();
      } else {
        throw new NumberFormatException((val != null) ? val.toString() : null);
      }
    }
    /**
