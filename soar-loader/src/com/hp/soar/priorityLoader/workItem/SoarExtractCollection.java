...
public class SoarExtractCollection extends SoarExtractElement {
	 ...
	 */
	 private void addCollectionElements(Element collectionElement) throws DfException, ProcessingException {
		 ...
		 //SMO : User Story 7650 - inclusion of company flag - Start
		 //SoarExtractXMLBuilder.addXMLElement(collectionElement,"company_info", dbService.getCompanyInfo(dbObj.getString("company")));
		 String company_info = "";
		 String copyright = "Hewlett-Packard Company";
		 
		Map companyInformation = dbService.getCompanyInformations(dbObj
				.getString("company"));
		if(!companyInformation.isEmpty())
		{	
		    company_info = companyInformation.keySet().toArray()[0]
				.toString();
		    copyright = companyInformation.get(company_info).toString();
		    
		}
			 
		SoarExtractXMLBuilder.addXMLElement(collectionElement, "company_info",
				company_info);
		 //SMO : User Story 7650 - inclusion of company flag - end
		...
		addCopyrightElement(collectionElement, copyright);
	...
	private void addCopyrightElement(Element collectionElement, String copyright)
			throws DfException {
		 ...
		 if (cpRtDate.equals("")) {
			 ...
			 Element hpCopyRightNoticeElement = SoarExtractXMLBuilder.addXMLElement(copyRightElement,"hp-copyright-notice","Copyright � " + cpRtDate + " "+copyright);
	...
	private String getCopyrightName() {
		String companyName = "Hewlett-Packard Company";
		return null;
	}

	private void addDmsDocumentsElement(Element collectionElement)
			throws DfException {
	...
	private boolean loadSoftwareItems(boolean isPhysicalMediaItems) throws DfException, ProcessingException, IOException {
		...
		// create the ProductNames cache for this collection
		ProductNames productNamesMap = new ProductNames();
					...
					if (dbService.isItemDuplicated(itemId, collectionId)) {
						...
						if ((lastExtractionTimeStamp == null) && (itemState.equals(OBSOLETE) || isSuspended)) {
							...
							if (isPhysicalMediaItems) {
								// ICM changes
								swItem = new PhysicalMediaExtractItem(dbService, eventId, objectId, isICM, icmRule, chronicleId, collectionId, itemId, 
										itemState, isSuspended, inSubscription, lastExtractionTimeStamp, productNamesMap);
							} else {
								// ICM changes
								swItem = new SoarExtractItem(dbService, eventId, objectId, isICM, icmRule, chronicleId, collectionId, itemId, itemState, 
										isSuspended, inSubscription, lastExtractionTimeStamp, productNamesMap); 
							}
							swItems.add(swItem);
