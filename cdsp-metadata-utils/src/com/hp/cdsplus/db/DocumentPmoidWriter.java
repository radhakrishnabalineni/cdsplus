...
public class DocumentPmoidWriter extends Thread{
	...
	static final int fileChangeCount = 1200000;
	...
	public void run() {
        ...
        synchronized (pmoidQueue){
        	pmoidQSize = pmoidQueue.size();
        }

        while (pmoidQSize > 0 || parsingData) {
        	...
        	if (pmoidQSize > 0) {
				...
				if (pmoidRowCount >= fileChangeCount) {
					closeFiles();
					openFiles();
					objCount = 0;
					pmoidRowCount = 0;
        ...
        ContentMetaDataProcessorDB.writingData = false;
	...
	private void writeDoc(DocumentBean docBean) {
		...
		try {
			...
			pmoidRowCount = pmoidRowCount + docBean.getPmoids().size();
