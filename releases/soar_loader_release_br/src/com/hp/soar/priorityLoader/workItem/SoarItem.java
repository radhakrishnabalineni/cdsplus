...
public class SoarItem extends SoarExtractElement {
	 ...
	 * @throws IOException 
	 */
	public SoarItem(SoarExtractionDBService dbService, String eventId, String objectId, boolean icmDriver, String icmRule, 
			String chronicleId, String collectionId, String itemId, String itemState, boolean isSuspended, 
			boolean collectionAccepted, Date lastExtractionTime) throws DfException, ProcessingException, IOException {
		// ICM changes
		super(dbService, objectId, icmDriver, icmRule, (itemState.equals(OBSOLETE) || isSuspended) ? ITEM_DELETED : ITEM_MODIFIED);
		...
		if (dbObj != null) {
			...
			// set shouldPublish based on if the item is obsolete or suspended
			shouldPublish = !(itemState.equals(OBSOLETE) || isSuspended);
			...
			//if(pushFTPtoo){
				//System.out.println("pushFTPtoo is ->"+pushFTPtoo+" hence loading the software files to push to ftp.");
				loadSWFiles(lastExtractionTime, eventId);
			//}else{
			//	System.out.println("pushFTPtoo is ->"+pushFTPtoo+" hence NOT loading the software files to push to ftp.");
			//}
	...
	private void addProductsSupportedElement(Element itemElement) throws  DfException {
		...
		ArrayList<ProductNameBean> nameBeans = ProductNames.getNameBeans(dbService.getSession(), collectionId, 
				dbObj.getString("product_group_oid"));
	...
	public void doDelete(HashSet<String> publishedLocations) throws ProcessingException {
		LoaderLog.info("SoarItem doDelete swFileSets size: " + swFileSets.size());
		for(String key : swFileSets.keySet()) {
			for(SoarExtractSWFile swFile : swFileSets.get(key)) {
					LoaderLog.info("SoarItem doDelete swFileSets col: " +collectionId+":"+itemId+":"+swFile.getIdentifier());
			}
		}
		for(String key : swFileSets.keySet()) {
			for(SoarExtractSWFile swFile : swFileSets.get(key)) {
	...
	public void doImmediateDelete(HashSet<String> publishedLocations) throws IOException, ProcessingException {
		LoaderLog.info("SoarItem doImmediateDelete swFileSets size: " + swFileSets.size());
		for(String key : swFileSets.keySet()) {
			for(SoarExtractSWFile swFile : swFileSets.get(key)) {
					LoaderLog.info("SoarItem doImmediateDelete swFileSets col: " +collectionId+":"+itemId+":"+swFile.getIdentifier());
			}
		}
		for(String key : swFileSets.keySet()) {
			for(SoarExtractSWFile swFile : swFileSets.get(key)) {
				swFile.doImmediateDelete(inSubscription, publishedLocations);
			}
		}
		// set the availability to false
	...
	public void doUpdates() throws ProcessingException {
		LoaderLog.info("SoarItem doUpdates swFileSets size: " + swFileSets.size());
		if (!updateCompleted) {
	...
	private void loadSWFiles(Date lastExtractionTime, String eventId) throws DfException, IOException {
		...
		for(String fileObjId : fileObjIds) {
			try {
				SoarExtractSWFile swFile = SoarExtractSWFile.getSWFile(dbService, dbObj.getBoolean("icm_driver"), dbObj.getString("icm_rule_id"), fileObjId, updateType, lastExtractionTime, eventId, collectionId, itemId);
	...
	public void getPublishedLocations(HashSet<String> publishedLocations) {
		LoaderLog.info("SoarItem getPublishedLocations swFileSets size: " + swFileSets.size());	
		for(String key : swFileSets.keySet()) {
			for(SoarExtractSWFile swFile : swFileSets.get(key)) {
					LoaderLog.info("SoarItem getPublishedLocations swFileSets col: " +collectionId+":"+itemId+":"+swFile.getIdentifier());
			}
		}
		if (inSubscription) {
