...
import com.hp.loader.single.loader.SingleHistoryToken;
import com.hp.loader.single.loader.SingleTokenHistoryHandler;
import com.hp.loader.single.loader.SinglePriorityLoader;
...
abstract public class SingleWorkItem implements SingleIWorkItem {
	...
	protected SingleIWorkItem prevItem;

	// Item that is happening after this item
	protected SingleIWorkItem postItem;

	// history token for the history update of this item
	protected SingleHistoryToken historyToken;
	...
	public SingleWorkItem( Long token, Integer priority, String eventType ) {
	 ...
	 * @param vals
	 * @throws SingleProcessingException
	 */
	public SingleWorkItem(Iterator<String> vIt) throws SingleProcessingException {
		valid = true;
		prevItem = null;
		postItem = null;
		historyToken = null;		
	...
	/* (non-Javadoc)
	 * @see com.hp.loader.workItem.SingleIWorkItem#exit()
	 */
	public void exit() {
		exiting = true;
	}
	...
	static public String getEvent(String eventType) {
		if (eventType.equalsIgnoreCase(SingleIWorkItem.EVENT_UPDATE)) {
			return SingleIWorkItem.EVENT_UPDATE;
		} else if (eventType.equalsIgnoreCase(SingleIWorkItem.EVENT_DELETE)) {
			return SingleIWorkItem.EVENT_DELETE;
		} else if (eventType.equalsIgnoreCase(SingleIWorkItem.EVENT_TOUCH)) {
			return SingleIWorkItem.EVENT_TOUCH;
		} else {
	 ...
	 * @see com.hp.loader.workItem.IWorkItem#manageDuplicate(com.hp.loader.workItem.WorkItem)
	 */
	static public void resolveDuplicate(SingleIWorkItem newWorkItem, SingleIWorkItem oldWorkItem) {
	...
	synchronized public void setPostItem(SingleIWorkItem postItem) {
	 ...
	 */
	synchronized public void setPrevItem(SingleIWorkItem prevItem) {
	 ...
	 */
	abstract public void load() throws SingleProcessingException;

	/* (non-Javadoc)
	 * @see com.hp.loader.workItem.IWorkItem#loadItem()
	 */
	final public boolean loadItem() throws SingleProcessingException {
		// setup the messaging
		...
		while(!exiting) {
			...
			try {
				...
				if((e instanceof SingleProcessingException) && ((SingleProcessingException)e).shouldRetry()){
					if(!SinglePriorityLoader.retryOnError){
					processMsg.setLength(baseMsgLength);
					processMsg.append(e.getClass().getName()).append(getFullTag());
					Log.error(processMsg.toString());
					SinglePriorityLoader.exitOnError(e);
					valid = false;
					return false;
					}
					else{
	 ...
	 */
	abstract public void logStatus(String status, Exception e) throws SingleProcessingException;
	 ...
	 */
	abstract public boolean store() throws SingleProcessingException;
	...
	private void doStore() throws SingleProcessingException, IOException, SingleHistoryException {
	 ...
	 */
	final public void storeItem() throws SingleProcessingException, IOException, SingleHistoryException {
		...
		while(!exiting) {
			try {
				...
				try {
					...
					logStatus(SingleWorkItem.SUCCESS);
				} catch (SingleProcessingException pe) {
					processMsg.setLength(baseMsgLength);
				...
				return;
			} catch (SingleHistoryException he) {
				...
				if (valid && !tooManyFails) {
					...
					ErrorLog.logStackTrace(e);
					try {
						logStatus(SingleIWorkItem.SAVE_FAILED, e);
					} catch (SingleProcessingException pe) {
	...
	public void createHistoryToken() {
		createHistoryToken(this, SingleTokenHistoryHandler.getInstance());
	}

	/* (non-Javadoc)
	 * @see com.hp.loader.workItem.IWorkItem#createHistoryToken(com.hp.loader.history.HistoryHandler)
	 */
	/*public void createHistoryToken(SingleIWorkItem item, SingleHistoryToken handler) {
		historyToken = new SingleHistoryToken(item, handler);
	}*/
	...
	private boolean logLoadException(Exception e) {
		boolean load = false;
		if (e instanceof SingleProcessingException) {
			load = ((SingleProcessingException)e).shouldRetry();
		} else {
		...
		if (load) {
			...
			try {
				logStatus(SingleIWorkItem.LOAD_FAILED, e);
			} catch (SingleProcessingException pe) {
	...
	public boolean schedule() {
		return SingleTokenHistoryHandler.getInstance().scheduleWorkItem(this);
	}
	
	protected String getFieldValue(Iterator<String> vIt, String fieldName) throws SingleProcessingException {
		String val = vIt.next();
		if (val == null) {
			throw new SingleProcessingException("Failed at field "+fieldName);
		}
		return val;
	}
