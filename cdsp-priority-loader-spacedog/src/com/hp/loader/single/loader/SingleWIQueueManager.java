 ...
 * @since       1.0
 */
public class SingleWIQueueManager {
  ...
  // workItem to process
  private TreeMap<Integer,LinkedList<SingleIWorkItem>> contentQueuesPool;

  // Index of the queues content,  used for fast lookup of duplicate entries
  private HashMap<String, SingleIWorkItem> wiQueuesIndex = null;
   ...
   */
  public SingleWIQueueManager(int numPriorities, int numWorkers) {
    contentQueuesPool = new TreeMap<Integer,LinkedList<SingleIWorkItem>>();
    for(int i=0; i<numPriorities; i++) {
      contentQueuesPool.put(new Integer(i), new LinkedList<SingleIWorkItem>());
    }
    wiQueuesIndex  = new HashMap<String, SingleIWorkItem>();
  ...
  public int queueSize(Integer priority) {
    LinkedList<SingleIWorkItem> l = contentQueuesPool.get(priority);
   ...
   * @see WorkItem
   */
  public void push(SingleIWorkItem workItem) {
   ...
   * @return
   */
  private SingleIWorkItem getWorkItem(LinkedList<SingleIWorkItem> list) {
    while(list.size() > 0) {
    	SingleIWorkItem retVal = list.removeFirst();
      synchronized(retVal) {
        if (retVal.isValid()) {
          ...
          // This workItem was set to invalid, so mark it as done
        	SingleIWorkItem postItem = retVal.getPostItem();
          if (postItem != null) {
   ...
   * @see WorkItem
   */
  public SingleIWorkItem pop(SingleIWorkItem lastWorkItem) {
	  SingleIWorkItem retVal = null;
    synchronized(this) {
      // remove the lastItem from the queueIndex
      // take this one out of the indexes as well
      if (lastWorkItem != null) {
        wiQueuesIndex.remove(lastWorkItem.getIdentifier());
      }
      ...
      for(LinkedList<SingleIWorkItem> list : contentQueuesPool.values()) {
