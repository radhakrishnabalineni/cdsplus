...
import com.hp.loader.single.loader.SingleManualRepublish;
 ...
 *
 */
public class SinglePriorityLoader  {
	...
	private static SinglePriorityLoader priorityLoader = null;
	...
	SingleTokenHistoryHandler historyHandler = null;
	
	// queueManager
	SingleWIQueueManager queueManager = null;
	...
	ArrayList<SingleWIWorker> workers = new ArrayList<SingleWIWorker>();

	// Source object providing workItems
	SingleSource source = null;
	 ...
	 * 
	 */
	public SinglePriorityLoader(ConfigurationReader config) throws ParserConfigurationException, SAXException, IOException, AddressException, SingleHistoryException {
		...
		queueManager = new SingleWIQueueManager(numPriorities, numWorkerThreads);
		...
		historyHandler = SingleTokenHistoryHandler.getInstance(historyFileName, numPriorities, queueManager, 
		...
		for(int i=0; i<numWorkerThreads; i++) {
			workers.add(new SingleWIWorker(queueManager, source));
		}
	...
	public void start() {
		...
		source = (SingleSource)getClassInstance(SOURCE_CLASS);
	}
	...
	public void exit() {
		...
		for(SingleWIWorker worker : workers) {
			worker.exit();
		}
	}
	...
	private int loadWorkOrder() throws SingleProcessingException {
		// Gets the list of work items
		ArrayList<SingleIWorkItem> workItemsList = source.getWorkItemsSince(historyHandler.getStartToken());
		...
		if (workItemsList.size() > 0) {
			...
			synchronized(queueManager) {
				for(SingleIWorkItem workItem : workItemsList) {
	...
	public SinglePriorityLoader() {
		// TODO Auto-generated constructor stub
	}
	
	public  void lodDataFromConcentra() {
		...
		while (!exiting) {
				...
				if (timeToSleep > 0) {
					if(batchMode && SingleManualRepublish.isManualRepubDone && queueManager.isEmpty()){
	 ...
	 * @return
	 */
	public SingleWIQueueManager getQueueManager() {
		return queueManager;
	}
	...
	protected  void initNonProcessedItems() throws IOException, SingleProcessingException {
	 ...
	 * @throws AddressException 
	 */
	static public synchronized SinglePriorityLoader getInstance(ConfigurationReader config) throws AddressException, ParserConfigurationException, SAXException, IOException, SingleHistoryException {
		if (priorityLoader == null) {
			priorityLoader = new SinglePriorityLoader(config);
	 ...
	 * @return
	 */
	static public SinglePriorityLoader getInstance() {
		return priorityLoader;
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			Enumeration<URL> configFiles = ClassLoader.getSystemResources(configFileName);
			if (configFiles.hasMoreElements()) {
				URL configLocation = configFiles.nextElement();
				File configFile = new File(configLocation.getPath());
				if (!configFile.canRead()) {
					System.out.println(configFile+ " is not readable.");
					System.exit(1);
				}
				config = new ConfigurationReader(configLocation.getPath());
			} else {
				System.out.println("Can't find configuration "+configFileName+" on classpath.");
				System.exit(1);
			}
			...
			SinglePriorityLoader loader = SinglePriorityLoader.getInstance(config);
			
			//SinglePriorityLoader loader = new SinglePriorityLoader();

			/*Thread t = new Thread(loader,"Loader");
			t.setDaemon(true);
			t.start();*/
			
			// remove rum method and call your own method
			
			loader.lodDataFromConcentra();
			...
			while(!stopFile.exists()) {
				if (pauseFile.exists()) {
					SinglePriorityLoader.pause();
					pauseFile.delete();
				} else if (resumeFile.exists()) {
					SinglePriorityLoader.resume();
					resumeFile.delete();
				}
				if (loader.isPaused()) {
	...
	public static void exitOnError(Throwable t){
			...
			SinglePriorityLoader.getInstance().exit();
			Log.info("Exited");
			System.exit(1);
	}
