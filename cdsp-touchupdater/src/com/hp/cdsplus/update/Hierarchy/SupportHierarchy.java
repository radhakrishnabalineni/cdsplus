 ...
 *
 */
public class SupportHierarchy extends Hierarchy {
	private ArrayList<String> supp_hier = new ArrayList<String>(6);
	
	//constructor can initialize the levels of the support tree as well as call super() to ensure they get the right objects they need
	public SupportHierarchy(UpdateManager up, UpdateThread upT){
		super(up, upT);
		
		supp_hier.add("PRODUCT_NUMBER"); // index 0
		supp_hier.add("PRODUCT_NAME");
		...
		supp_hier.add("PRODUCT_SERIES");
		supp_hier.add("PRODUCT_BIGSERIES");
		supp_hier.add("SUPPORT_SUBCATEGORY");
		supp_hier.add("SUPPORT_CATEGORY"); // index 5
	}
	...
	public void climb(DBObject obj) throws MongoDataException, MongoQueryException{
		...
		for(int i=0; i < supp_hier.size(); i++){// taking the current level the hierarchy collection tells us this OID is at, go through the support array i made instantiating the hierarchy to traverse the tree
			if(level.equalsIgnoreCase(supp_hier.get(i))){
		...
		for(int i=matched_level_index; i < supp_hier.size(); i++){ // so now, starting at the level we found above, go up the tree to category level and find OIDs
			if( supp_hier.get(i).equalsIgnoreCase("PRODUCT_NAME") ){ // handle the SUPPORT_NAME OIDs too at this point
				...
				if( level_oid != null && !level_oid.equals("") ){
					t.getLogger().debug("The SUPPORT_NAME_OID for " + level + " " + obj.get("_id").toString()  + " is " + level_oid);
			...
			level_oid = obj.get(supp_hier.get(i)+"_OID").toString(); // fetch the OID for this level so we can figure out what GUIDs are tagged to it
			
			if( level_oid != null && !level_oid.equals("") ){
				t.getLogger().debug("The " + (supp_hier.get(i)+"_OID").toString() + " for " + level + " " + obj.get("_id").toString()  + " is " + level_oid);
				findGUIDs(level_oid); // call superclass's function to handle finding the GUIDs for this level's OID
			} else{
				throw new MongoDataException("Level OID for " + level + " and oid " + obj.get("_id").toString() + " is empty or nonexistant. Check the data. skipping this OID's processing.");
			}
		...
		t.getLogger().info("********Done climbing support hierarchy for " + obj.get("_id").toString() + "********");
	}
